...
if __name__ == '__main__':
    # 第一种方式运行用例
    args = ["-v", script_path.replace("\\", "/"),
            "-s",
            "--alluredir", xml_report_path.replace("\\", "/"),
            "--html={}".format(summary_report_path.replace("\\", "/")),
            '--capture=no'
            ]
    pytest.main(args)
    cmd = "allure generate {} -o {} --clean" .format(xml_report_path, detail_report_path)
    # reportResult = batch(cmd)
    os.system("allure serve " + xml_report_path)
