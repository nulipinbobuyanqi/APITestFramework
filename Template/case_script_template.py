...
class TestApiCycleInfoQueryList:
    ...
    def setup_method(self):
        with allure.step("第一步：初始化用例所需的数据环境"):
            logger.info("执行SQL文件，初始化用例所需的数据环境")
            ...
            if len(all_sql_list) != 4:
                ...
                logger.info("有初始化用例环境所需要运行的SQL")
                for i in range(5, len(all_sql_list)):
                    logger.info("当前正在获取第{}个需要初始化环境的SQL".format(i))
                    sql_content = all_sql_list[i].split("\n")[0]
                    logger.info("第{}个SQL={}".format(i - 4, sql_content))
                    if sql_content != "":
                        # 对SQL语句进行切片，获取SQL语句类型
                        sql_type = sql_content.split(" ")[0]
                        logger.info("第{}个SQL的类型是：{}".format(i - 4, sql_type))
                        # 执行SQL查询，得到查询结果
                        sql_acture_result = send_sql_request(run_env, db_env_data, database_type, sql_type, sql_content)
                        logger.info("执行第{}个SQL，得到的结果是：{}".format(i - 4, sql_acture_result))
                        if sql_type.lower() == "select":
                            ...
                            for field in field_list:
                                ...
                                if field == "*":
                                    logger.info("取所有字段，无意义，不加入依赖字段信息")
                                    break
                            logger.info("执行第{}个SQL，得到的结果，其对应的字段列表是：{}".format(i - 4, field_list))
                        if sql_type.lower() == "select":
                            ...
                            if len(sql_acture_result) == 1:
                                ...
                                for j in range(len(list_result)):
                                    logger.info("正在遍历第{}个查询结果值".format(j + 1))
                                    logger.info("第{}个查询结果值为：{}".format(j + 1, list_result[j]))
                                    ...
                                    for n in range(len(list_result[j])):
                                        ...
                                        if key in list(dict_sql_depent.keys()):
                                            logger.info("{}字段已存在依赖信息里，不能继续将它加入到依赖信息".format(key))
                                        else:
                                            dict_sql_depent[key] = value
                            else:
                                logger.info("第{}个初始化select类型的SQL，查询出来的结果要么为空，要么有多条结果，本套代码仅支持一条查询结果，所以，无法将结果写入依赖信息".format(i - 4))
                        else:
                            logger.info("第{}个SQL不是查询语句，无需加入依赖信息".format(i - 4))
                            logger.info("\n")
                            continue
                    else:
                        logger.info("第{}个SQL为空，跳出，继续遍历下一个SQL语句".format(i - 4, sql_content))
                        logger.info("\n")
                        continue
                    logger.info("\n")
            else:
                logger.info("没有初始化用例环境所需要运行的SQL")

    def teardown_method(self):
        with allure.step("第十二步：清除用例执行后的数据环境"):
            logger.info("执行SQL文件，清除用例执行后的数据环境")
            all_sql_list = get_sql_data(sql_path, clear_sql_file_name)
            # 因为前四行是注意事项，所以长度设置的是4
            if len(all_sql_list) != 4:
                # 数据库类型
                database_type = all_sql_list[4].split(" ")[-1].replace("\n","")
                logger.info("有清除用例环境所需要运行的SQL")
                for i in range(5, len(all_sql_list)):
                    logger.info("当前正在获取第{}个需要清除环境的SQL".format(i))
                    sql_content = all_sql_list[i].split("\n")[0]
                    logger.info("第{}个SQL={}".format(i - 4, sql_content))
                    if sql_content != "":
                        # 对SQL语句进行切片，获取SQL语句类型
                        ...
                        if sql_type.lower() != "select":
                            ...
                            logger.info("清除用例环境的SQL语句类型，不能是select ！")
                    else:
                        logger.info("第{}个SQL为空，跳出，继续遍历下一个SQL语句".format(i - 4, sql_content))
                        logger.info("\n")
                        continue
    ...
    def test_template(self, every_case_data):
        ...
        with allure.step("第二步：发起HTTP请求"):
            ...
            logger.info("当前接口：{} 的请求参数是：{}".format(interface_address, parameters))
            logger.info("当前接口：{} 的请求参数的数据类型是：{}".format(interface_address, type(parameters)))
            # 判断是否需要上传文件
            if need_file:
                logger.info("当前接口：{} 需要上传文件".format(interface_address))
                # 发起接口请求
                file_name = interface_parameters["file"]
                logger.info("当前接口：{} 上传的文件是：{} ".format(interface_address, str(file_name)))
                ...
                logger.info("当前接口：{} 不需要上传文件".format(interface_address))
                # 执行接口，得到接口返回值
            ...
            logger.info("当前接口：{} 的接口实际返回值是：{}".format(interface_address, api_acture_result))
        ...
        with allure.step("第四步：进行接口校验"):
            ...
            logger.info("当前接口：{} 的接口校验信息是：{}".format(interface_address, api_check_info))
            if api_check_info is not None:
                ...
                if api_check_res:
                    ...
                    logger.info("当前接口：{} 的接口校验通过！".format(interface_address))
                    pytest.assume(api_check_res is True)
                else:
                    allure.attach(name="接口校验结果", body="FAIL")
                    logger.info("当前接口：{} 的接口校验失败！".format(interface_address))
                    pytest.assume(api_check_res is True)
            else:
                allure.attach(name="接口校验信息", body="空")
                logger.info("当前接口：{} 的接口校验信息为空！无法进行校验！请检查！".format(interface_address))
                pytest.assume(api_check_info is not None)

        with allure.step("第五步：发起SQL请求"):
            # 获取发起查询前的各项信息
            sql_info = every_case_data["case_step"]["sql_check"]
            logger.info("当前接口：{} 的SQL请求信息是：{}".format(interface_address, sql_info))
            if sql_info is not None:
                ...
                logger.info("当前接口：{} 的SQL实际返回值是：{}".format(interface_address, sql_acture_result))
            else:
                allure.attach(name="SQL校验信息", body="空")
                logger.info("当前接口：{} 的SQL校验信息为空！无法进行校验！请检查！".format(interface_address))
        ...
        with allure.step("第六步：进行SQL校验"):
            ...
            logger.info("当前接口：{} 的SQL校验信息是：{}".format(interface_address, sql_check_info))
            # 准备进行校验
            if sql_check_info is not None:
                ...
                if sql_check_res:
                    ...
                    logger.info("当前接口：{} 的SQL校验通过！".format(interface_address))
                    pytest.assume(sql_check_res is True)
                else:
                    allure.attach(name="SQL校验结果", body="FAIL")
                    logger.info("当前接口：{} 的SQL校验失败！".format(interface_address))
                    pytest.assume(sql_check_res is True)
            else:
                allure.attach(name="SQL校验信息", body="空")
                logger.info("当前接口：{} 的SQL校验信息为空！无法进行校验！请检查！".format(interface_address))
        ...
        with allure.step("第七步：判断用例执行结果是通过还是失败"):
            if api_check_res and sql_check_res:
                ...
                logger.info("当前接口：{} 接口校验和SQL校验都通过！".format(interface_address))
                # 说明用例通过，将结果设置为PASS
                case_res = True
                ...
                logger.info("当前接口：{} 接口校验和SQL校验，至少有一个未通过！".format(interface_address))
                # 说明用例失败，将结果设置为FAIL
                case_res = False
                pytest.assume(case_res is True)
